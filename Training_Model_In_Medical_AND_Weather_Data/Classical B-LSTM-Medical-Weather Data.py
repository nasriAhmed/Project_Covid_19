...
from keras.layers import Dense, Bidirectional, Dropout, Activation
...
df_new.to_csv(r'..\test.csv',index = False, header=True)
...
model = Sequential()
model.add(Bidirectional(LSTM(units = 40 , return_sequences=True , input_shape = (train_x.shape[1],1))))
model.add(Bidirectional(LSTM(units = 40 , return_sequences = True)))
model.add(Dropout(0.2))
model.add(Bidirectional(LSTM(units= 40 , return_sequences = False)))
model.add(Dense(25))
model.add(Dense(1))
dot_img_file = '../Dataset/model_BLSTM_quantum_weather.png'
